...
class MockLiteLlm(LiteLlm):
    ...
    async def generate_content_async(self, llm_request: LlmRequest, stream: bool = False, **kwargs) -> AsyncGenerator[LlmResponse, None]:
        ...
        if "quantum" in user_text:
            ...
            function_call = types.FunctionCall(
                ...
                args={
                    "requests": [
                        ...
                        {"tool_name": "create_structured_task", "args": {
                            "title": "Quantum Error Correction",
                            "description": "Research and implement a basic surface code simulation.",
                            "acceptance_criteria": ["Read surface code papers", "Write python simulation", "Verify error rates"],
                            "priority": 3,
                            "complexity": "large"
                        }}
                    ]
            ...
            yield LlmResponse(content=types.Content(role="model", parts=[types.Part(text="I'll search for quantum and plan a task.", function_call=function_call)]))
            return
        ...
        if "delay" in user_text:
            ...
            yield LlmResponse(content=types.Content(role="model", parts=[types.Part(text="Running parallel delay tasks.", function_call=function_call)]))
            return
        ...
        if "improve our tests" in user_text:
            ...
            yield LlmResponse(content=types.Content(role="model", parts=[types.Part(text="I've researched testing trends. Creating a structured task now.", function_call=function_call)]))
            return

        yield LlmResponse(content=types.Content(role="model", parts=[types.Part(text="Mock response from Elephant.")]))
